...
namespace Steering
{
    ...
    public class Flee : Behaviour
    {
        ...
        public override Vector3 CalculateSteeringForce(float dt, BehaviourContext context)
        {
            ...
            if (Vector3.Distance(context.m_position,target.transform.position) < context.m_settings.m_fleeRange)
            {
                context.m_positionTarget = target.transform.position; // Set the target position to flee away from
            }
            else
            {
